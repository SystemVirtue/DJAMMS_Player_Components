...
export function useVideoPlayer(config: VideoPlayerConfig) {
  ...
  const getVideoSource = useCallback((video: Video): string => {
    ...
    if (isElectron) {
      // Extract the actual file path (remove file:// prefix if present)
      let normalizedPath = videoPath.replace(/\\/g, '/');
      if (normalizedPath.startsWith('file://')) {
        normalizedPath = normalizedPath.substring(7); // Remove 'file://' prefix
        console.log('[VIDEO] Removed file:// prefix, normalized path:', normalizedPath);
      }
      
      // In dev mode, use djamms:// protocol (registered in main process)
      // This avoids file:// URL issues with webSecurity and CORS
      if (isDevMode) {
        // DO NOT use encodeURIComponent() - it double-encodes slashes
        // Use the path directly, the protocol handler will handle it
      ...
      // In production, use file:// URLs
      // If it already has file://, return as-is, otherwise add it
      if (videoPath.startsWith('file://')) {
        return videoPath;
      }
      
      // In production, use file:// URLs
      // Ensure proper URL encoding for file paths with spaces and special characters
      // file:// URLs need forward slashes, not backslashes, and spaces should be encoded
